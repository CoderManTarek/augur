...
def grabreg(records, repo_id, dsfile):
    ...
    proc = safe_command.run(subprocess.Popen, "dosocs2 generate " + str(records[0][0]), shell=True, stdout=PIPE, stderr=PIPE)
    ...
    pope = safe_command.run(subprocess.Popen, "dosocs2 print " + str(charvarerr) + " -T " + dsfile, shell=True, stdout=PIPE, stderr=PIPE)
